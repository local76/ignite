...
sed "s/TEMPLATE_VERSION/$VERSION/g" rstart.spec > rpmbuild/SPECS/rstart.spec

# Create source tarball
tar --exclude='rpmbuild' --exclude='target' --exclude='.git' -czf rpmbuild/SOURCES/rstart-$VERSION.tar.gz -C ../.. .

# Run rpmbuild locally
rpmbuild -ba rpmbuild/SPECS/rstart.spec --define "_topdir $(pwd)/rpmbuild"

# Copy output to dist/packages
cp rpmbuild/RPMS/*/*.rpm ../../dist/packages/rstart.rpm
