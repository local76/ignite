...
echo "=== Releasing ignite v$VERSION ==="
...
if ! git diff --cached --quiet; then
    git commit -m "release: ignite $VERSION"
else
...
git tag -a "v$VERSION" -m "ignite v$VERSION"
...
if command -v gh &> /dev/null; then
    ...
    if [ ${#ASSETS[@]} -gt 0 ]; then
        gh_args=("release" "create" "v$VERSION" "${ASSETS[@]}" "--title" "ignite v$VERSION" "--generate-notes")
...
echo "=== Release ignite v$VERSION completed successfully! ==="
